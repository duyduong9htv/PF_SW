...
for k = 1:size(obs, 1)
    if ddist(obs(k, :), w.rcvLocs(1, :)) < 50e3
        inds = [inds; k]; 
    end    
end


inds1 = randsample(inds, size(obs, 1), true); 

particles = obs(inds1, :); %set the initial observations to be the particles 
...
%              figure(6); plot(x0, y0, 'ko', 'linewidth', 1); 
    ...
    for ii = 1:size(obs, 1)
        ...
        theta_x = calBearings(particles(ii, :), w.rcvLocs(k+step, :)); 
        alpha = 90 - theta_x; 
        a21 = tand(alpha - halfbw); 
        a22 = tand(alpha + halfbw); 
